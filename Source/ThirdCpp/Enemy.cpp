...
void AEnemy::Tick(float DeltaTime)
{
	...
	auto animInst
		= Cast<UEnemyAnimInstance>(GetMesh()->GetAnimInstance());
	...
	if (_chasedTarget != nullptr && animInst->State == EEnemyState::Locomotion) {
		...
		if (enemyController) enemyController->MakeAttackDecision(_chasedTarget);
	}
...
bool AEnemy::CanAttack() {
	...
	return (_AttackCountingDown <= 0.0f &&
		enemyAnimInst->State == EEnemyState::Locomotion);
}
...
void AEnemy::Chase(APawn* targetPawn) {
	...
	if (targetPawn != nullptr && enemyAnimInst->State == EEnemyState::Locomotion)
	{
		auto enemyController = Cast<AEnemyController>(GetController());
		if (enemyController) {
			enemyController->MoveToActor(targetPawn, 100.0f);
		}	
	}
